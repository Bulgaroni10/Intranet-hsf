...
def api_listar_notificacoes(request):
    itens = listar_notificacoes(request.user, limite=30)
    return JsonResponse({
...
def api_marcar_notificacao_lida(request, notificacao_id):
    get_object_or_404(NotificacaoUsuario, id=notificacao_id, usuario=request.user)
    marcar_como_lida(request.user, notificacao_id)
    return JsonResponse({'ok': True, 'nao_lidas': contar_nao_lidas(request.user)})
